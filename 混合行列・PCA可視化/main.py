"""
野菜判定AI 精度評価スクリプト (混同行列+PCA可視化版) エントリーポイント
...
主な処理の流れ:
    ...
    3. evaluators で指定した手法(既定: 混同行列+PCA可視化)により評価
...
from evaluators.confusion_pca_evaluator import ConfusionPCAConfig
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="野菜判定AIの精度評価 (既定: 混同行列+PCA可視化)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--method", type=str, default="confusion_pca", choices=list_methods(),
        help="評価手法 (confusion_pca=混同行列+PCA可視化, "
             "intra_inter_cosine=クラス内・クラス間コサイン類似度[教師なし], "
    ...
    # --- 混同行列+PCA可視化用オプション ---
    parser.add_argument(
        "--classifier", type=str, default="knn", choices=["knn", "linear_probe"],
        help="[confusion_pca] 混同行列を作るために使う分類器",
    )
    parser.add_argument(
        "--no-highlight-misclassified", action="store_true",
        help="[confusion_pca] PCA散布図で誤分類サンプルを強調表示しない",
    )
    ...
    parser.add_argument(
        ...
        help="[silhouette/knn/confusion_pca(knn使用時)] 距離指標 (cosine, euclidean など)",
    )
    parser.add_argument(
    ...
    # --- 交差検証・分類器共通オプション ---
    parser.add_argument(
        "--cv", type=str, default="loo", choices=["loo", "kfold"],
        help="[confusion_pca/linear_probe/knn] 交差検証方法",
    )
    parser.add_argument("--n-splits", type=int, default=5, help="kfoldの分割数")
...
def _build_config(args: argparse.Namespace):
    if args.method == "confusion_pca":
        return ConfusionPCAConfig(
            ...
            highlight_misclassified=not args.no_highlight_misclassified,
        )
